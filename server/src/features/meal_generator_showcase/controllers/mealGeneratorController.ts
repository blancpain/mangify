...
const getRecipes = async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
  const { numberOfMeals, diet } = req.query;

  // NOTE: type casting here should be OK since frontend user inputs are hardcoded as number and string...
  const recipes = await mealGeneratorService.getRecipes(Number(numberOfMeals), diet as string);
  if (recipes) {
